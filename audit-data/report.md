...
### [H-3] Upgrading the contract cases the storage slots for `ThunderLoan::s_feePrecision` to collapse. Freezing protocol and breaking functionality.

**Description:** The `ThundeLoan.sol` has two fee variables as follows:
...
However, when we upgrade the contract `ThunderLoanUpgraded.sol`, these variables are changed as:
